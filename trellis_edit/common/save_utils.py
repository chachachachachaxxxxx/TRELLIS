...
def save_outputs(
    ...
    for sample_idx in range(num_samples):
        ...
        if not skip_glb and "mesh" in outputs:
            # Check if we have gaussian for texture, otherwise export mesh-only GLB
            if "gaussian" in outputs:
                print(f"Exporting GLB with texture for sample {sample_idx}...")
                release_cuda_memory()
                ...
                print(f"✓ Saved GLB with texture: {glb_path}")
                del glb
                release_cuda_memory()
            else:
                # Export mesh-only GLB without texture
                print(f"Exporting mesh-only GLB for sample {sample_idx}...")
                release_cuda_memory()
                mesh = outputs["mesh"][sample_idx]
                ...
                print(f"✓ Saved mesh-only GLB: {glb_path}")
                del trimesh_mesh
                release_cuda_memory()
