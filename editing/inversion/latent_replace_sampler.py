"""Sparse Latent Replacement RF Sampler for latent_replace_union ablation."""
...
def apply_sparse_latent_replacement(
    ...
    Args:
        ...
        cached_latent: Cached latent from inversion (SparseTensor)
...
class SparseLatentReplaceRFSampler(SecondOrderRFSampler):
    """RF Sampler with sparse latent replacement for latent_replace_union ablation.

    This sampler caches intermediate latents during inversion, then replaces
    specific latent features during denoising to preserve source regions.
    """
    ...
    def invert_with_cache(
        ...
        Returns:
            ...
            - latent_cache: Dict mapping time keys to cached latents
        """
        latent_cache = {}
        ...
        for t_curr, t_next in tqdm(t_pairs, desc="RF inversion (cache stage2 latents)", disable=not verbose):
            ...
            # Cache the latent at t_next (on CPU to save GPU memory)
